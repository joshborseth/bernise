...
export const ttsChunkLimit = 20_000;
...
const rewriteProse = (text: string): string => {
  ...
  value = value.replace(/\[groan\]/gi, "nngh");
  value = value.replace(/\[sigh\]/gi, "hhah");
...
export const splitForTts = (text: string): ReadonlyArray<string> => {
  ...
  if (trimmed.length <= ttsChunkLimit) {
    return [trimmed];
  }

  const parts: Array<string> = [];
  let rest = trimmed;
  while (rest.length > ttsChunkLimit) {
    const window = rest.slice(0, ttsChunkLimit);
    ...
    const at = breakAt >= 200 ? breakAt + 1 : ttsChunkLimit;
