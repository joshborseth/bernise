...
const settingsWriteEpochAtom = Atom.make(0).pipe(Atom.keepAlive);

const applySettings = (settings: HarnessSettings, patch?: SettingsPatch): HarnessSettings =>
  new HarnessSettings({
    codex: settings.codex,
    persona:
      patch?.persona === null
        ? defaultBernisePersona
        : typeof patch?.persona === "string"
          ? patch.persona
          : resolvePersona(settings.persona),
  });
...
export const bootSettingsAtom = BerniseRpc.runtime
  .atom((get) =>
    Effect.gen(function* () {
      ...
      if (get.once(settingsWriteEpochAtom) === 0) {
        get.set(settingsAtom, applySettings(settings));
      }
...
export const updateSettingsAtom = BerniseRpc.runtime
  .fn((patch: SettingsPatch, get) =>
    Effect.uninterruptible(
      Effect.gen(function* () {
        get.set(settingsWriteEpochAtom, get.registry.get(settingsWriteEpochAtom) + 1);
        const client = yield* BerniseRpc;
        const next = yield* client("UpdateSettings", patch);
        get.set(settingsAtom, applySettings(next, patch));
        return next;
      }),
    ),
  )
